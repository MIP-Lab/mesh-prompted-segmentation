...
class Sam(nn.Module):
    def __init__(self,
                 backbone_cfg,
                 neck_cfg=None,
                 read_out_head_cfg=None,
                 train_cfg=None,
                 test_cfg=None,
                 init_cfg=None):
        super().__init__()
        
        self.backbone = ResNet3d(**backbone_cfg)
        self.backbone.init_weights()
        if neck_cfg is not None:
            self.neck = FPN3d(**neck_cfg)
        self.criterion = torch.nn.CrossEntropyLoss().cuda()
        self.read_out_head = FPN3d(**read_out_head_cfg)
    ...
    def single_fine_loss(self, fine_feat, coarse_feat, fine_grid, coarse_grid, fine_vaild, coarse_valid, *args):
        ...
        fine_local_grid = self.meshgrid3d(fine_feat.shape[2:]).to(fine_feat.device)  # z y x
        ...
        if intersection_y[0] > intersection_y[1] or intersection_x[0] > intersection_x[1] or intersection_z[0] > \
            ...
            points1_select = points1_select.to(pos1_mm_overlap.device)
            points2_select = points2_select.to(pos2_mm_overlap.device)
